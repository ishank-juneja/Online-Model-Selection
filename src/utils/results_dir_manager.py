...
class ResultDirManager:
    ...
    def remove_location(self, loc_name: str):
        if self.loc_exists(loc_name):
            os.rmdir(self.get_abs_path(loc_name))
        return
    ...
    def next_path(self, loc_name: str, prefix: str, postfix: str):
        ...
        ret_path = full_pattern_pth % b
        # If a directory create the dir since it won't already exist
        if '.' not in ret_path:
    ...
    def scrape_loc_for_glob(self, loc_name: str, pattern: str):
        if self.loc_exists(loc_name):
            paths = glob.glob(os.path.join(self.get_abs_path(loc_name), pattern))
    ...
    # Checks if a string could be a filename
    def is_file_name(self, mystr: str):
        if '.' in mystr:
            return True
        else:
            return False

    def list_dirs_in_dir(self, abs_path_to_dir: str, pattern: str = None, return_sorted: bool = True):
        object_list = self.list_dir_objects(abs_path_to_dir, pattern, return_sorted)
        files_list = []
        for object in object_list:
            # object can only be either file_name or a dir
            if not self.is_file_name(object):
                files_list.append(object)
        return files_list

    def list_dir_files(self, abs_path_to_dir: str, pattern: str = None, return_sorted: bool = True):
        object_list = self.list_dir_objects(abs_path_to_dir, pattern, return_sorted)
        files_list = []
        for object in object_list:
            if self.is_file_name(object):
                files_list.append(object)
        return files_list

    def list_dir_objects(self, abs_path_to_dir: str, pattern: str = None, return_sorted: bool = True):
        """
        :param abs_path_to_dir: absolute path of the dir wanted listed
        :param pattern: glob if looking for a certain pattern ex: images*.npy
        :param return_sorted: whether returned strings should be sorted in natural/human order
        :return: list of all objects (files and dirs) in the passed abs_path_to_dir (file)
        """
        object_list = []
    ...
    def get_name_sans_extension_from_path(self, file_path: str):
        return self.strip_path_of_extension(self.get_name_from_path(file_path))
