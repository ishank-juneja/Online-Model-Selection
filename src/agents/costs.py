import torch
import numpy as np
from arm_pytorch_utilities.linalg import batch_quadratic_product
...
class QuadraticCost:
    ...
    def compute_cost(self, state, action=None):
        ...
        """
        if action is None:
            return batch_quadratic_product(state - self.goal, self.Q)

        return batch_quadratic_product(state - self.goal, self.Q) + batch_quadratic_product(action, self.R)
