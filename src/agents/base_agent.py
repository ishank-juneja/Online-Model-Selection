...
class BaseAgent(metaclass=ABCMeta):
    ...
    def __init__(self, device: str = 'cuda:0'):
        ...
        # The dataset holder objects are Agent and SimpModBook. SimpModLib does not hold data
        # - - - - - - - - - - - - - - - - - - - -
    ...
    def step(self):
        ...
        gt_state = info['state']
        # Reshape gt_state to row vector
        gt_state = gt_state[None, :]

        total_reward += rew

        # Update rob_state tracked by Simp Mod Lib to latest
        ...
        self.episode_history['gt_rob_state'].append(self.model_lib.rob_state.cpu().detach().numpy())
    ...
    def reset_episode(self):
        ...
        self.episode_history['action'].append(actions)
        self.episode_history['gt_state'].append(gt_state)
        self.episode_history['gt_frame'].append(obs)
        self.episode_history['gt_rob_state'].append(self.model_lib.rob_state.cpu().detach().numpy())
        # TODO: Reflect any changes in self.data_keys here
    ...
    def save_episode_data(self, npz_file_path: str):
        """
    ...
    def load_episode_data(self, ep_file_path: str):
        """
        Load an npz file containing the logged data from an episode
        :return:
        """
        raw_ep_history = np.load(ep_file_path, allow_pickle=True)
        # Dict to hold formatted version of raw agent_ep_history
        agent_ep_history = {}

        # Rearrange loaded data back into agent data and simple model specific data
        for key in self.data_keys:
            agent_ep_history[key] = raw_ep_history[key]

        smodels_ep_history = {}

        for smodel in self.model_lib.model_names:
            # Allocate dict to hold data attrs for this smodel
            smodels_ep_history[smodel] = {}
            # Ignore pycharm warning, npz objects loaded via np.load have a files attr.
            for key in raw_ep_history.files:
                key_split = key.split('_')
                if smodel == key_split[0]:
                    new_key = '_'.join(key_split[1:])
                    smodels_ep_history[smodel][new_key] = raw_ep_history[key]

        return agent_ep_history, smodels_ep_history
